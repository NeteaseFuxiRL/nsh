...
class ZMQ(Msg):
    def __init__(self, config, index):
        ...
        self.sender.bind(f'ipc://{PATH_IPC}/{index}.game')
        self.receiver = self.context.socket(zmq.PAIR)
        self.receiver.connect(f'ipc://{PATH_IPC}/{index}.env')
...
class Redis(Msg):
    def __init__(self, config, index):
        ...
        self.key_send = f'nsh.game{index}'
        self.key_receive = f'nsh.env{index}'
...
class RabbitMQ(Msg):
    def __init__(self, config, index):
        ...
        self.key_send = f'game{index}'
        self.key_receive = f'env{index}'
...
class IPC(Msg):
    def __init__(self, config, index):
        ...
        self.sender = posix_ipc.MessageQueue(f'/{prefix}.g2e', flags=posix_ipc.O_CREAT, max_message_size=max_message_size)
        self.receiver = posix_ipc.MessageQueue(f'/{prefix}.e2g', flags=posix_ipc.O_CREAT, max_message_size=max_message_size)

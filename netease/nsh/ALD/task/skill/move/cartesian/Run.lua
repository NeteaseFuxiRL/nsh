...
local Skill = require('ALD/role/skill/move/Cartesian')

local Super = require('ALD/task/skill/move/Cartesian')
local Class = {}
rawset(_G, 'ALD/task/skill/move/cartesian/Run', Class)
...
function Class:Insert()
    for _, item in ipairs(self.directions) do
        self.role:InsertSkill(Skill.Create(self.role, item.name, item.direction))
    end
end

...
local Class = {}
rawset(_G, 'ALD/task/feature/enemy/StatusGroup', Class)

function Class.Create(...)
    local self = setmetatable(Super.Create(...), { __index = setmetatable(Class, { __index = Super})})
    local status_group = {}
    for _, item in ipairs(python:Iter2Table(builtins.iter(config.items('nsh_enemy_status_group')))) do
    ...
    for _, key in ipairs(python:Iter2Table(builtins.iter(builtins.str.split(config.get('nsh', 'enemy_status_group'))))) do
...
function Class:Insert(feature)
    ...
    for _, item in ipairs(python:Iter2Table(builtins.iter(self.status_group.items()))) do
        ...
        feature['enemy_status_group_' .. key] = enemy:StatusOr(tags) and 1 or 0
    end
end

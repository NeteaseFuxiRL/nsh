...
local Class = {}
rawset(_G, 'ALD/task/feature/enemy/Status', Class)

function Class.Create(...)
    local self = setmetatable(Super.Create(...), { __index = setmetatable(Class, { __index = Super})})
    self.status = python:Iter2Table(builtins.iter(builtins.str.split(config.get('nsh', 'enemy_status'))))
    return self
end
...
function Class:Insert(feature)
    ...
    for _, name in ipairs(self.status) do
        feature['enemy_status_' .. name] = g_StatusMgr:GetStatus(target, EPropStatus[name]) ~= 0
    end
end

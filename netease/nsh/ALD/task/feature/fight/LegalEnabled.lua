...
local Class = {}
rawset(_G, 'ALD/task/feature/fight/LegalEnabled', Class)
...
function Class:Insert(feature)
    for _, skill in ipairs(self.role.skill) do
        if util.IsInstance(skill, Fight) then
            local name = skill.name .. '_legal_enabled'
            if skill.Enabled and not skill:Enabled() then
                feature[name] = -1
            else
                feature[name] = skill:IsLegal() and 1 or 0
            end
        end
    end
end

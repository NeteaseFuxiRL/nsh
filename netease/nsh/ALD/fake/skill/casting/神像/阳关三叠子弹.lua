...
local Class = {}
rawset(_G, 'ALD/fake/skill/casting/神像/阳关三叠子弹', Class)
...
function Class:Hurt800113()
    local player = self.player
    local target = player.target
    FightActions.mFireAtt.Formula(player, target, {}, self.context, {0.23 * 1.3 * FA5a, FA5b, 1, 0})
    return true
end

function Class:Hurt800114()
    local player = self.player
    local target = player.target
    FightActions.mFireAtt.Formula(player, target, {}, self.context, {0.58 * 1.3 * FA5a, FA5b, 1, 0})
    return true
end
